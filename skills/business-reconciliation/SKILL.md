...
4. 使用 `cloud.process.execute` 执行冻结脚本；输入分别绑定 `invoices.csv` 与 `payments.csv`，只申明 `reconciliation.json` 和 `reconciliation.csv` 输出。固定 Node 沙箱，无网络、无本地工作区访问。执行前等待平台对精确脚本、输入对象和输出范围的审批；不能把用户澄清或普通聊天当作批准。
