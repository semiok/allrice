...
The export response reports rendered page count when the isolated renderer succeeds. The workbench shows up to eight bounded PNG page previews of the exact source version. Check headings, table wrapping and page breaks there; original DOCX bytes are retained. Rendering alone is not visual approval. If conversion is unavailable, return the file and the actual limitation.
