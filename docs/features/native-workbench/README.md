...
右栏直接复用固定官方 `ui-sidebar-right/SidebarRight.module.css`，与原生 DockLayout 配合；来源及原样字节加入同步账本（25 份来源），不另写滑动动画。Allrice 布局保持三个 Grid 轨道，关闭时右轨为 0，原生 Dock 内容按同一 300ms 曲线从右侧滑入，正文同步让位。关闭时保留当前 Dock 内容与意见，离开会话仍检查未保存内容；窄屏、全屏和拖拽继续使用原有组件。仅对首次异步挂载补 starting-style，并修正减少动态效果的覆盖优先级。
