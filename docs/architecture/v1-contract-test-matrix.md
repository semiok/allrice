...
| Employee version      | Rice publishes a checksum-addressed manifest with exact skills | published fields cannot mutate/delete; legacy basic version cannot execute           | MET-45        |
| Employee assignment   | explicit default and exact version create a Session/Run        | list order, another user's assignment or cross-tenant version grants nothing         | MET-45        |
| Employee run          | Run freezes prompt/provider/skills and atomically writes reply | retry duplicates no Message/Run; failure/cancel persists visible terminal evidence   | MET-45        |
