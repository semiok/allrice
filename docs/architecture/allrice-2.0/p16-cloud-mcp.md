...
4. 发布员工版本时，还必须显式授权 `cloud.mcp.call`、service 身份、`secret:use`，并在 `connectorRefs` 加入 `mcp.<connection-id>`。连接安装本身不授予员工调用权。
5. 新 Run 冻结确切 connection revision、tool revision/schema digest、grant revision 和 opaque credential reference。旧 Run 不自动继承新工具或新授权。
...
8. Dispatch 与私有 lease journal 原子落库；START receipt 的 `mayExecute` 是唯一一次 tools/call 门禁。每次网络前及运行中心跳都复查 job lease、当前会员、连接、工具授权、schema、精确审批和 root 状态。
...
按顺序执行：应用迁移 → 安全配置 Web/Worker 同一主密钥 → 开启 Dev 功能开关 → 创建自有测试连接 → 发现 → 逐工具授权 → 员工版本显式绑定连接和工具 → 发布并创建新 Run → 审查并批准一次调用。不得为验收借用个人 Linear/GitHub 或真实租户账户。
