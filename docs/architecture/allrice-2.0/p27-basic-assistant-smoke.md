...
**当前状态（2026-09-14）**：干净 `7161caad6e74ca6410ddc2539fa36ce85a07e782` 的一次真实 Gemini 双助手在 70.1 秒后失败（一子 completed、一子 partial、未知用量保留）。补首因诊断后，新的干净 `9254fece7de28e4c40e28578a7031b859cc103be` 试验在根首次请求收到固定 SDK 分类 `finish/SERVER`，没有创建子助手；新首因与同次调用的持久准入/价格回执成功关联，不据此倒推旧失败的原因。两次资源均确认清理，不自动整场重放；真实成功验收仍未通过。详见[本轮收尾记录](./b6-closeout-20260914.md)。脚本存在、预检退出 0、合成协议或 PG 测试通过，均不代表 P27、B6 或 GA 完成。

完整任务准备、配置冻结、生产 Worker、月额度和成功后的下一条普通聊天另由 [P27 Worker 连续任务验收](./p27-worker-smoke.md) 补充，独立授权、独立收据，不能用本 adapter-only 子集代替。
