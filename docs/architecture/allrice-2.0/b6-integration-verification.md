...
已在独立 closeout 工作树实现订阅冻结身份、0097 不可变路由证明、订阅 N/A 计量、普通/助手准入、0098 同账号额度缓存与后台卡片。未知 Token、权限、租约、取消和无重放检查保留；旧 API 代码与历史记录保留。真实额度只读查询于 10:43 成功，主 Codex 周窗口已用 70%；真实双助手及后续普通任务、最终 UI/联合矩阵尚待本轮证据。未部署 Dev/Prod，未开放真实租户助手。具体边界见 [Codex 订阅验收口径](./p27-codex-first-acceptance.md)。
