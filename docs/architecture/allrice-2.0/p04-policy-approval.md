...
同一 operation 的旧审批记录是不可绕过的失效记录：改输入摘要或把控制从 Ask 改为 Allow，都不能抹去旧拒绝、撤销、到期或版本失效。请求、决定及消费在审计写入后再次用数据库当前时间复核有效期；审计锁等待跨过有效期时，审批变化与审计一起回滚。真实 PostgreSQL 锁等待测试分别覆盖这三个事务出口。
