...
会话右缘的轮次导航复用同版官方 `TurnNavigator` 组件、原样 CSS 和有界预览函数；发布入口尚未单独导出该组件，因此采用已有源码同步方式，只替换宿主类型导入。Allrice 负责已有消息/运行关联、中文提示、消息锚点及回看时暂停自动跟随，不另写导航条、悬停预览或虚拟列表。至少两轮且正文宽度足够时默认展示，成果栏挤占空间或手机窄屏时沿用官方隐藏规则；预览不调用模型、不新增 Token 消耗。来源、类型补丁与依赖已登记，后续官方提供独立组件导出时优先改为直接导入。
