...
const nativeSkillToolCapabilities: Readonly<Record<string, SkillCapability>> = {
  ...
  'browser.run': 'network:outbound',
  'wechat.article.search': 'network:outbound',
  'wechat.article.read': 'network:outbound',
  'market.quote': 'network:outbound',
  'market.history': 'network:outbound',
  'workspace.memory.remember': 'storage:write',
  'workspace.export.create': 'storage:write',
...
export async function assignEmployeeVersion(
  ...
  await sql`
    ...
    where organization_id = ${context.organizationId}
      ...
      and employee_version_id <> ${update.employeeVersionId}
  `;
  await sql`
    insert into allrice_audit_events (
      organization_id, workspace_id, actor_id, action, resource_type,
