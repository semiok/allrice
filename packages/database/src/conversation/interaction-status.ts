...
export async function getInteractionStatus(
  ...
  return db.begin(async (tx) => {
    ...
    return {
      runTimings: await sessionRunTimings(tx, context, sessionId),
...
/** Ordinary ChatFlow timing is independent of the optional artifact workbench. */
export async function getSessionRunTimings(
  context: WorkbenchPrincipal,
  sessionId: string,
  db = getDatabase(),
) {
  return db.begin(async (tx) => {
    await assertWorkbenchSession(tx, context, sessionId);
    return { runTimings: await sessionRunTimings(tx, context, sessionId) };
  });
}
async function sessionRunTimings(
  tx: TransactionSql,
  context: WorkbenchPrincipal,
  sessionId: string,
) {
  ...
  const runs = await tx<{ run_id: string }[]>`
      ...
      order by (er.run_id=(select active_run_id from allrice_conversation_runtimes where session_id=${sessionId})) desc nulls last,
