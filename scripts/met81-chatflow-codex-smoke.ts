...
try {
  ...
  const eventsUrl = new URL(
    `/api/v1/runs/${second.runId}/events?workspaceId=${binding.workspace_id}&contract=chatflow-v2&format=json`,
    baseUrl,
  );
  ...
  if (
    ...
    throw new Error('ChatFlow v2 did not expose a canonical DSH event stream');
  }
  ...
  process.stdout.write(
    `${JSON.stringify({
      ...
      eventContract: 'chatflow-native-v2',
    })}\n`,
  );
} finally {
