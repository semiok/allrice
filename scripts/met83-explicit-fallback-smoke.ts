...
const fallbackRows = await sql<{ connection_id: string; catalog_id: string }[]>`
  select c.id as connection_id, m.id as catalog_id
  ...
  where p.provider_key = 'minimax' and p.enabled
...
if (!fallbackTarget) {
  await sql.end();
  throw new Error('a ready MiniMax Provider route is required');
}
...
try {
  ...
  if (
    ...
    decision.provider !== 'openai-compatible' ||
