...
export interface P27WorkerFixtureCleanup {
  globalDatabaseClosed: boolean;
  databaseEnvironmentRestored: boolean;
  fixture: AssistantFixtureCleanupProof | null;
}

export class P27WorkerFixtureError extends Error {
  constructor(
    readonly code: string,
    readonly cleanup?: P27WorkerFixtureCleanup,
  ) {
    super(code);
  }
}
...
function requireFixture(ok: unknown, code: string): asserts ok {
  if (!ok) throw new P27WorkerFixtureError(code);
}
...
export async function createP27WorkerFixture() {
  requireFixture(!owned, 'P27_WORKER_FIXTURE_ALREADY_OWNED');
  requireFixture(!process.env.DATABASE_URL, 'P27_WORKER_AMBIENT_DATABASE');
  requireFixture(
  ...
  let closePromise: Promise<P27WorkerFixtureCleanup> | undefined;
  let closed = false;
  const close = () =>
    (closePromise ??= (async () => {
      closed = true;
      const proof: P27WorkerFixtureCleanup = {
      ...
      if (!disposed)
        throw new P27WorkerFixtureError(
  ...
  try {
    ...
    requireFixture(relations?.safe === true, 'P27_WORKER_RELATION_SCOPE');
    const [contracts, employees, models, queue, governance, config] =
      await Promise.all([
        ...
        import('../../../packages/database/src/providers/model-governance.ts'),
    ...
    const [catalog] = await db<{ id: string; provider_id: string }[]>`
      ...
      where p.provider_key='gemini' and p.auth_mode='api_key'
        and m.model='gemini-3.8-flash'`;
    requireFixture(catalog, 'P27_WORKER_GEMINI_CATALOG_MISSING');
    const catalogId = catalog.id;
    const price = createP27GeminiPriceBinding({
      connectionId,
      catalogId,
      at: new Date().toISOString(),
    });
    const pricingCatalog = contracts.AssistantPriceCatalogSchema.parse({
      version: 1,
      catalogVersion: price.snapshot.catalogVersion,
      entries: [price.snapshot.price],
    });
    const { P27_GEMINI_RUN_LIMITS: runLimits } =
      await import('./p27-assistant-pricing.ts');
    ...
    await db.begin(async (tx) => {
      ...
      await tx`insert into allrice_organizations(id,slug,name) values(${organizationId},${`p27-worker-${organizationId}`},'P27 isolated')`;
      ...
      await tx`insert into allrice_employees(id,organization_id,workspace_id,employee_key,name) values(${employeeId},${organizationId},${workspaceId},'p27-worker','P27 isolated')`;
      ...
      await tx`insert into allrice_model_connections(id,provider_id,scope,name,credential_reference,base_url,status)
        values(${connectionId},${catalog.provider_id},'platform','P27 isolated Gemini','deployment:gemini-default',null,'ready')`;
    ...
    async function prepare(prompt: string, assistants: boolean) {
      requireFixture(!closed && !preparing, 'P27_WORKER_FIXTURE_NOT_IDLE');
      requireFixture(
        process.env.DATABASE_URL === databaseUrl,
      ...
      try {
        ...
        requireFixture(pending?.count === 1, 'P27_WORKER_UNEXPECTED_QUEUE');
        const leaseMs = 30000;
    ...
    return {
      ...
      pricingCatalog,
      runLimits,
      async prepareAssistantTask(prompt: string) {
        requireFixture(!firstAttempted, 'P27_WORKER_FIRST_ALREADY_ATTEMPTED');
        firstAttempted = true;
        const task = await prepare(prompt, true);
        firstRunId = task.runId;
        return task;
      },
      async prepareOrdinaryTask(input: { afterRunId: string; prompt: string }) {
        requireFixture(
          !closed && !preparing && !ordinaryAttempted && !ordinaryChecking,
          'P27_WORKER_ORDINARY_NOT_IDLE',
        );
        requireFixture(
          firstRunId && input.afterRunId === firstRunId,
        ...
        try {
          ...
          >`
          select r.state,j.status,d.status as decision_status,d.cost_cents as decision_cost,l.cost_cents as ledger_cost,
            d.usage_complete as decision_complete,l.usage_complete as ledger_complete
          from allrice_runs r join allrice_jobs j on j.run_id=r.id
          join allrice_route_decisions d on d.run_id=r.id
          join allrice_model_usage_ledger l on l.route_decision_id=d.id
          where r.id=${firstRunId} and r.organization_id=${organizationId} and d.organization_id=${organizationId} and l.organization_id=${organizationId}`;
          const row = rows[0];
          requireFixture(
            rows.length === 1 &&
              row?.state === 'succeeded' &&
              row.status === 'succeeded' &&
              row.decision_status === 'succeeded' &&
              row.decision_complete &&
              row.ledger_complete &&
              row.decision_cost !== null &&
              row.ledger_cost === row.decision_cost &&
              Number(row.ledger_cost) > 0,
          ...
          return await prepare(input.prompt, false);
        } finally {
    ...
    throw new P27WorkerFixtureError(
      error instanceof P27WorkerFixtureError
        ? error.code
...
export type P27WorkerFixture = Awaited<
  ReturnType<typeof createP27WorkerFixture>
>;
export type P27PreparedWorkerTask = Awaited<
  ReturnType<P27WorkerFixture['prepareAssistantTask']>
>;
