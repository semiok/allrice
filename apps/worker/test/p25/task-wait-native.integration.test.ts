...
describe('MET153 native durable question boundary', () => {
  it('releases the original process, adopts the exact answer after restart and dispatches the continuation once', async () => {
    ...
    try {
      ...
      await expect
        .poll(() => notices.find((n) => n.method === 'session.user-question'), {
          timeout: 15000,
        })
        .toBeTruthy();
      ...
      expect((await recovered.answerWait(answer)).proof).toEqual(receipt.proof);
      expect(model.requests).toHaveLength(1);
