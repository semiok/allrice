...
suite(
  'MET-147 UX01-A full tenant workbench (synthetic HTTP, no model)',
  { timeout: 20_000 },
  () => {
    ...
    it('UX01-B bridges missing configuration to the real pairing dialog and refreshes on return', async () => {
      ...
      try {
        ...
        await expect
          .poll(() => f.state.readinessRequests)
    ...
    it('UX01-B revalidates after settings, protects members, and never treats errors or wrong scope as ready', async () => {
      ...
      try {
        ...
        await dialog
          ...
          .waitFor();
        expect(
          await dialog.getByRole('button', { name: /^准备/ }).count(),
        ).toBe(0);
    ...
    it('restores the selected Session after reload and removes stale links for new work', async () => {
    ...
    it('uses a narrow drawer, traps/restores focus, preserves drafts across resize, and has no horizontal overflow', async () => {
      ...
      try {
        ...
        await f.page.keyboard.press('Escape');
        expect(
          await f.entry.evaluate((e) => e === document.activeElement),
        ).toBe(true);
        expect(await f.panel.count()).toBe(0);
