// DSH native DisclosureRow; MIT, see upstream.json. Only imports/class-name joining adapted.
import { type KeyboardEvent, type MouseEvent, type ReactNode } from 'react';
// Only string class names are used by this primitive; no extra dependency.
const clsx = (...names: (string | undefined)[]) =>
  names.filter(Boolean).join(' ');
import { IconChevronDownOutline14 } from './QueueIcons';
  ...
  const collapsedLeading = previewChevron ? (
    <>
      ...
      <IconChevronDownOutline14
        className={clsx(chevronClassName, css.chevronHover)}
      />
    </>
  ) : (
    icon
    ...
    <div className={clsx(css.root, className)} data-open={open || undefined}>
      ...
      >
        {expandable && !rowExpands ? (
          ...
          <span className={clsx(css.leading, leadingClassName)}>{leading}</span>
        )}
