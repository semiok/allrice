...
import type { RemoteFailure, FilesBodyHostProps, TranslateNS } from './contracts'
import {
...
import type { WorkspaceDirectoryEntry } from './contracts'
...
export type FilesBodyProps = FilesBodyHostProps & FilesInjected
...
export function orderEntries(entries: readonly WorkspaceDirectoryEntry[]): WorkspaceDirectoryEntry[] {
  return [...entries].sort((left, right) => {
    ...
    return group !== 0 ? group : byName.compare(left.displayName ?? left.name, right.displayName ?? right.name)
  })
}
    ...
    return (
      <li className={css.item} data-files-entry="directory" data-files-path={path}>
        <button type="button" className={css.row} aria-expanded={expanded} onClick={() => { tree.onToggle(parent, path) }}>
          ...
          <span className={css.name}>{entry.displayName ?? entry.name}</span>
        </button>
    ...
    return (
      <li className={css.item} data-files-entry="file" data-files-path={path}>
        <button type="button" className={css.row} onClick={() => { tree.onOpen(path) }}>
          <FileTypeIcon kind={classifyFileType(entry.displayName ?? entry.name)} size={16} className={css.fileIcon} />
          <span className={css.name}>{entry.displayName ?? entry.name}</span>
        </button>
      </li>
    )
