 ...
 */
import type { RemoteResult, SessionId } from './contracts'
    ...
    return {
      ...
      start(tabId, root, signal) {
        ...
        roots.set(tabId, new DirectoryNode(root,
          ...
          (path, error) => {
            if (!signal.aborted) actions.failed(tabId, path, { code: 'file/unavailable', message: error instanceof Error ? error.message : String(error) })
          }, signal,
        ).open())
      },
